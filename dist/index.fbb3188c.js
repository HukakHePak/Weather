...
async function updateWeather(cityName) {
    ...
    _apiJs.requestWeather(cityName).then((response)=>{
        ...
        console.log(error);
        _viewJs.controls.notify('Unknow city');
    });
}
function changeLike() {
...
_viewJs.NODES.FAVORITES.addEventListener('add', (event)=>{
    ...
    _storageJs.storage.addFavorite(city);
    event.detail.node.lastElementChild.addEventListener('click', ()=>{
        _viewJs.controls.removeFavorite(city);
    });
...
_viewJs.NODES.FAVORITES.addEventListener('remove', (event)=>{
    _storageJs.storage.removeFavorite(event.detail.city);
    changeLike();
});
...
const controls = {
    notify (msg) {
        ...
        setTimeout(()=>deactivate(NOTIFICATION)
        , 2000);
    },
    setLike (liked) {
...
function convertDate(milisec) {
    ...
    return {
        time: _dateFns.format(date, 'HH:mm'),
        date: _dateFns.format(date, 'dd LLL')
    };
}
